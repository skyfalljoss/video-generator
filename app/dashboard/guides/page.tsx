...
const steps = [
  ...
  },
  {
    ...
    bullets: [
      ...
      "Matching images are generated or sourced automatically",
  ...
  },
  {
    ...
    description: "If you connected a YouTube account and enabled it for your Series, the video is uploaded automatically after generation.",
    bullets: [
    ...
    <div className="p-6 md:p-10 max-w-4xl mx-auto">
      ...
      <div className="mb-10">
        ...
        <p className="mt-3 text-zinc-500 dark:text-zinc-400 text-lg max-w-2xl">
          Everything you need to go from zero to publishing your first AI-generated short video.
      ...
      <div className="space-y-8">
        {steps.map((step, i) => (
          <div
            key={step.number}
            className="relative rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-7 shadow-sm hover:shadow-md transition-shadow"
          >
            {/* Connector line */}
            {i < steps.length - 1 && (
              <div className="absolute left-[52px] top-full h-8 w-px bg-zinc-200 dark:bg-zinc-800" />
            )}

            <div className="flex items-start gap-5">
              {/* Step number + icon */}
              <div className="flex flex-col items-center gap-2 shrink-0">
                <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 shadow-sm">
                  {step.icon}
                </div>
                <span className="text-[11px] font-bold text-zinc-300 dark:text-zinc-600 tracking-widest">
                  {step.number}
                </span>
              </div>

              <div className="flex-1 min-w-0">
                <h2 className="text-lg font-bold text-zinc-900 dark:text-white mb-1">{step.title}</h2>
                <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-4 leading-relaxed">{step.description}</p>
                ...
                <ul className="space-y-2 mb-5">
                  {step.bullets.map((b) => (
                    <li key={b} className="flex items-start gap-2.5">
                      ...
                      <span className="text-sm text-zinc-700 dark:text-zinc-300">{b}</span>
                    </li>
                  ))}
                </ul>

                {/* Tip box */}
                <div className="rounded-xl bg-indigo-50 dark:bg-indigo-950/40 border border-indigo-100 dark:border-indigo-900 px-4 py-3">
                  <p className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 uppercase tracking-wide mb-1">
                    💡 Pro Tip
                  </p>
                  <p className="text-sm text-indigo-800 dark:text-indigo-200 leading-relaxed">{step.tip}</p>
                </div>
              </div>
            </div>
          </div>
